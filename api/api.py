...
class BudgetViewset(ReadOnlyModelViewSet):
    ...
    def expenses_by_country(self, request):
        ...
        for country in countries:
            ...
            expenses = Expense.objects.filter(budget__country=country, budget__year=year, code__in=codes,
                                              group='functional')
            budget = BudgetSummary.objects.filter(budget__year=year, budget__country=country).values(
                'expense_functional_budget').first()
