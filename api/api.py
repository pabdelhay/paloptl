...
class BudgetViewset(ReadOnlyModelViewSet):
    ...
    def total_expense(self, request, pk=None):
        ...
        for budget_summary in bs:
            ...
            if budget_summary.budget.currency != base_currency:
                if budget_summary.expense_functional_budget:
                    amount = budget_summary.expense_functional_budget / rate
                else:
                    amount = budget_summary.expense_organic_budget / rate
            else:
                amount = budget_summary.expense_organic_budget
    ...
    def transparency_index(self, request, pk=None):
        ...
        year = request.query_params.get('year', 2020)
        country = request.query_params.get('country', 'angola')
        t_index = TransparencyIndex.objects.filter(year = year, country__slug = country).first()
        return Response(t_index.transparency_index)
