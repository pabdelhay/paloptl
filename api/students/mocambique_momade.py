...
class MocambiqueMomadeViewset(ReadOnlyModelViewSet):
    model = Budget
    ...
    def year_percent_get_parameter(self, request, pk=None):
        ...
        q_budget_summary = BudgetSummary.objects.filter(budget__country__slug=slug)

        list_disc = {}
        list_result = []

        for c in q_budget_summary:
            if c.expense_functional_budget > 0 and c.expense_functional_execution:
                result = {
                    "percent": c.expense_functional_execution / c.expense_functional_budget,
                    "year": c.budget.year
                }
    ...
    def expense_detail_country(self, request, pk=None):
        ...
        country = request.query_params.get('code')
